...
// bounces back to /google.html with the tokens in the URL hash. The
// hash never reaches the server — only the browser reads it, then
// stores the tokens in localStorage. Mirrors api/whoop-callback.js.
//
// One request covers all three services (Calendar/Gmail/Drive) at
// once, since the authorize URL in google.html requests all three
// scopes together — one token pair, one connect flow.
//
...
export default async function handler(req, res) {
  ...
  // whoop-callback.js — Google sends the browser back to whatever
